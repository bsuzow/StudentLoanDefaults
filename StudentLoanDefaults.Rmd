...
The total number of students in the FY2012 repayment cohort is `r round(sum(sc1415.all$CDR3_DENOM,na.rm=TRUE)/1000000,1)` million. Of these, `r round(sum(sc1415.net$CDR3_DENOM)/1000000,1)` million who had attended `r nrow(sc1415.net)` schools are included in this study.  Their average default rate is `r mean(sc1415.net$CDR3)` with the standard deviation of `r sd(sc1415.net$CDR3)`.
...
While the # of predictors recommended is `r nvmaxCV$bestTune`, a dip is observed at 19. The RMSE delta between 19 and `r nvmaxCV$bestTune` nvmax levels is negligible. In the spirit of keeping the model as simply as possible, we will use 19 variables. 
